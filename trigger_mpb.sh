...
CRON_DIR=$HOME/cron
# The local directory where you have checkout out Simpetus/mpb-nightly-recipe.git
...
# The name of the file that holds the most recent NanoComp/mpb.git commit hash
COMMIT_FILENAME=latest_mpb_commit.txt
...
LATEST_COMMIT=$(git ls-remote git://github.com/NanoComp/mpb.git refs/heads/master | cut -f 1)
...
if [ "${LAST_KNOWN_COMMIT}" != "${LATEST_COMMIT}" ]; then
    ...
    pushd ${RECIPE_DIR}/mpb-nightly-recipe
    git checkout master
